    ...
    <div class="login-form-wrapper">
      ...
      <form @submit.prevent="e => submit(e)">
        ...
        <div class="input-wrapper">
          ...
          <input v-model="email" type="text" id="email" @click="setErrorMessage('')">
        </div>
        <div class="input-wrapper">
          <label for="psw">Salasana:</label>
          <input v-model="password" type="password" id="psw" @click="setErrorMessage('')">
        </div>
    ...
    async submit(e: Event) {
      ...
      if (error?.length) {
        ...
        if (error === ERROR.WRONG_PASSWORD) {
          ...
          this.setErrorMessage('Virhe kirjautuessa palveluun. Yritä uudelleen.')
        }
      } else {
