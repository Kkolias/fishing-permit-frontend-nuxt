    ...
    <div class="form-wrapper">
      ...
      <form @submit.prevent="e => submit(e)">
        ...
        <div class="input-wrapper">
          ...
          <input v-model="firstName" type="text" id="firstName" @click="setErrorTextsDefault()">
          <ErrorHover :errorMessage="errorTexts.firstName" />
        </div>
        <div class="input-wrapper">
          <label for="lastName">Sukunimi:</label>
          <input v-model="lastName" type="text" id="lastName" @click="setErrorTextsDefault()">
          <ErrorHover :errorMessage="errorTexts.lastName" />
        </div>
        <div class="input-wrapper">
          <label for="email">Sähköposti:</label>
          <input v-model="email" type="text" id="email" @click="setErrorTextsDefault()">
          <ErrorHover :errorMessage="errorTexts.email" />
        </div>
        <div class="input-wrapper">
          <label for="psw">Salasana:</label>
          <input v-model="password" type="password" id="psw" @click="setErrorTextsDefault()">
...
export default {
  ...
  methods: {
    ...
    async signUp() {
      ...
      const error = await apiUser.signUp({ firstName, lastName, email, password })

      if (error?.length) {
        if (error === ERROR.EMAIL_IN_USE) {
          this.setErrorTextByKey('email', error)
        } else {
          this.setOverViewErrorMessage('Uuden käyttäjän luominen epäonnistui. Yritä uudelleen.')
        }
      } else {
        this.$router.push("/")
      }
